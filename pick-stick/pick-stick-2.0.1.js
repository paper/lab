 ...
 * 
 * 2011-6-21
 * rotate 直接支持rad(弧度)，这样就不必使用deg(度)了。
 */
...
function PickSticks(obj){
	...
	function getRandom(begin,end){
		return parseInt( random()*(end-begin)+begin,10 );
	};
	
	//得到弧度
	function getRadRandom(begin,end){
		return parseFloat( random()*(end-begin)+begin ).toFixed(3);
	};
	...
	function createOneStick(){
		...
		tempDiv.setAttribute('data-rad',rad);
		...
		setStyle(tempDiv,{
			...
			WebkitTransform:'rotate('+rad+'rad)',
			MozTransform:'rotate('+rad+'rad)'
		});
	...
	function compareStick(A,B){
		var Arad=+A.getAttribute('data-rad'),
			Brad=+B.getAttribute('data-rad'),
			Awidth=width,
			Bwidth=width,
			Aheight=height,
			Bheight=height,
